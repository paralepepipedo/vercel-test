...
function cargarMisiones() {
  apiFetch('/api/admin?action=misiones_banco')
    .then(function (data) {
      if (!data) return;
...
function cargarEstadoEvento() {
  apiFetch('/api/admin?action=evento_activo')
    .then(function (data) {
      STATE.evento = (data && data.evento) ? data.evento : null;
      renderEventoBanner();
    })
    .catch(function () {
      // Si falla (ej: claves no existen aún en BD), simplemente no hay evento
      STATE.evento = null;
      renderEventoBanner();
    });
}
...
// HELPERS API
// ============================================
// ── Obtener token fresco de Clerk (evita 401 por rotación) ──
async function getToken() {
  try {
    // Clerk expone getToken() en su instancia global
    if (window.Clerk && window.Clerk.session) {
      const t = await window.Clerk.session.getToken();
      if (t) return t;
    }
  } catch (_) { }
  return window.CLERK_TOKEN || '';
}
