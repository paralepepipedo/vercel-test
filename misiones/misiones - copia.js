...
function accionMision(id, tipo) {
  ...
  // Si tiene url_destino → navegar
  if (mision.url_destino) {
    toast('🎯 ' + mision.descripcion, 'info');
...
function actualizarProgresoDiario() {
  ...
  setTimeout(function () {
    ...
    if (ring) {
      ...
      ring.style.stroke = pct >= 1 ? 'var(--green)' : pct >= 0.5 ? 'var(--cyan)' : 'var(--violet)';
    }
...
function iniciarTimer() {
  function tick() {
    var ahora = new Date();
    // Diario: cuenta hasta medianoche
    var manana = new Date(ahora); manana.setHours(24, 0, 0, 0);
    var diff = manana - ahora;
...
function actualizarTimerSemanal() {
  ...
  var sub = document.getElementById('timerSub');
  var el = document.getElementById('timerReinicio');
  if (label) label.textContent = 'SEMANA TERMINA';
  if (sub) sub.textContent = 'Las misiones se renuevan cada lunes';

  // Calcular días restantes hasta el lunes
  var ahora = new Date();
  var diaSem = ahora.getDay() || 7; // 1=lunes … 7=domingo
  var diasRest = 8 - diaSem; // días hasta el próximo lunes
  if (el) {
    el.textContent = diasRest + ' día' + (diasRest !== 1 ? 's' : '') + ' restante' + (diasRest !== 1 ? 's' : '');
    el.style.color = diasRest <= 2 ? 'var(--red)' : 'var(--cyan)';
  }
}
