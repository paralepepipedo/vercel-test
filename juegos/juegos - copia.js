...
function cargarDatos() {
  // 1. Cargar catálogo de juegos
  fetch(BASE_URL + '/api/juegos?action=catalogo', {
    headers: { 'Authorization': 'Bearer ' + window.CLERK_TOKEN }
  })
    .then(function (r) { return r.ok ? r.json() : null; })
    .then(function (data) {
      if (data && data.juegos && data.juegos.length) {
        STATE.juegos = data.juegos;
        renderJuegos();
      }
    })
    .catch(function () { });

  // 2. Cargar estadísticas del día + niveles por juego
  fetch(BASE_URL + '/api/juegos', {
    headers: { 'Authorization': 'Bearer ' + window.CLERK_TOKEN }
  })
    .then(function (r) { return r.ok ? r.json() : null; })
    .then(function (data) {
      ...
      console.log('[juegos.js] niveles_juegos recibido:', JSON.stringify(STATE.niveles_juegos, null, 2));
...
function renderJuegos() {
  ...
  grid.innerHTML = lista.map(function (j) {
    ...
    if (j.tiene_niveles) {
      ...
      var esMaestro = nivelActual >= (nd.nivel_maximo || (j.juego_id === 'coinclik' ? 5 : 10));

      // Clampear para que nunca muestre más del máximo
      ...
      if (esMaestro) {
        ...
        if (faltan <= 0) {
          // No debería pasar si la API resetea bien, pero por si acaso
          hintTxt = '✅ Listo';
        } else {
          hintTxt = faltan + ' para nv.' + (nivelActual + 1);
        }
      }
      ...
      nivelHTML =
        ...
        '<span class="juego-nivel-badge">Nv.' + nivelActual + '/' + (nd.nivel_maximo || (j.juego_id === 'coinclik' ? 5 : 10)) + '</span>' +
...
function lanzarJuego(id, url) {
  // Guardar datos para que el juego los use al completarse
  sessionStorage.setItem('juego_id', id);
  if (window.CLERK_TOKEN) {
    sessionStorage.setItem('juego_token', window.CLERK_TOKEN);
  }
    ...
    .then(function (data) {
      ...
      var html = data.mapa.map(function (n) {
        ...
        if (n.estado === 'completado') estadoTxt = 'Hecho';
          ...
          // Si ya completó los intentos requeridos (nivel listo para subir), mostrar "✅ Listo"
